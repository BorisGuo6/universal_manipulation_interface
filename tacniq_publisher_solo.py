...
import math
import rospy
from std_msgs.msg import Int16MultiArray, MultiArrayDimension, Float64, Int64
import usb
import re
...
class Tacniq(object):
    def __init__(self):
        ...
        if self.devR is None:
            ...
            rospy.logerr("Right finger not found")
            success = False
        if not success:
        ...
        self.time_publisher = rospy.Publisher("tacniq/time", Int64, queue_size=1)
        self.rotation_angle_pub = rospy.Publisher('/tactile/rotation_angle', Float64, queue_size=100, tcp_nodelay=True)
    ...
    def get_angle(self, mat):
        THRES = 200
        edge_idxs = []
        if isinstance(mat[0], float):
            return 0
        for col_idx in range(len(mat[0])):
            if (3485 - mat[0][col_idx] > THRES):  # find the edge
                edge_idxs.append((0, col_idx))
                continue

            for row_idx in range(len(mat) - 1):
                if (mat[row_idx][col_idx] - mat[row_idx + 1][col_idx] > THRES):  # find the edge
                    edge_idxs.append((row_idx + 1, col_idx))
                    break

        if len(edge_idxs) <= 1:
            return 0
        xs = [coord[1] for coord in edge_idxs]
        ys = [coord[0] for coord in edge_idxs]
        result = linregress(xs, ys)
        # print(result.slope)
        return math.atan2(result.slope, 1)
        #return math.atan2(edge_idxs[-1][0] - edge_idxs[0][0], edge_idxs[-1][1] - edge_idxs[0][1])


    def run(self):
        right_data = self.max_adc_value - self.read_device(self.devR)
        rospy_time_ns = rospy.Time.now().to_nsec()
        # time_ns = time.time_ns()
        # print(rospy_time_ns, type(rospy_time_ns))
        # print(self.sensor_data2ros(right_data), rospy_time_ns, time_ns)
        self.right_publisher.publish(self.sensor_data2ros(right_data))
        self.time_publisher.publish(rospy_time_ns)
        rotation_msg = Float64()
        rotation_msg.data = self.get_angle(right_data)
        self.rotation_angle_pub.publish(rotation_msg)

if __name__=='__main__':
    rospy.init_node('tacniq_ros_publisher')
