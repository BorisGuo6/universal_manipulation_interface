...
class TmiEnv:
    def __init__(self, 
        ...
        rospy.init_node("_TMI")

        # Search for GelSight devices
        ...
        grippers: List[WPGController] = list()
        for rc in robots_config:
            if rc['robot_type'].startswith('ur5'):
                ...
                this_robot = FrankaInterpolationController(
                    shm_manager=shm_manager,
                    robot_ip=rc['robot_ip'],
                    frequency=200, ## reduce speed to 10
                    Kx_scale=1.0,
        ...
        # GelSight and TAC-02 attributes
        self.gss = gss
    ...
    def get_obs(self) -> dict:
        ...
        # GelSight and TAC-02 observations
        tactile_obs = dict()
        assert len(self.gss) == 1, "TMI supports only one GelSight camera source"
        for gs in self.gss:
            try:
                gs.capture()
                gs.publish()
            ...
            resize_tf_gelsight = get_image_transform(
                ...
                out_res=self.obs_image_resolution,
            )
            gs.img = resize_tf_gelsight(gs.img)
            tactile_obs["gelsight0_rgb"] = gs.img
            tactile_obs["tac02_0"] = gs.tacniq_data
            
            #cv2.imshow('gsmini{}'.format(gs.src), gs.img) # For sanity check
