...
from umi.real_world.wsg_binary_driver import WPGBinaryDriver
...
class WPGController(mp.Process):
    def __init__(self,
        ...
        super().__init__(name="WPGController")
    ...
    def start(self, wait=True):
        ...
        if wait:
            self.start_wait()
        if self.verbose:
            print(f"[WPGController] Controller process spawned at {self.pid}")
    ...
    def run(self):
        # start connection
        try:
            with WPGBinaryDriver(
                hostname=self.hostname, 
                port=self.port) as WPG:
            ...
            self.ready_event.set()
            if self.verbose:
                print(f"[WPGController] Disconnected from robot: {self.hostname}")